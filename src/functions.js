...
//pega tudo da matriz e junta num array so
...
const wonGame = board => fields(board).filter(pendding).length === 0
  //const wonGame = board => fields(board).filter(f => (f.mined && f.flagged) || (!f.mined && f.opened)).length == fields(board)
const showMines = board => fields(board).filter(f => f.mined).forEach(f => f.opened = true);
